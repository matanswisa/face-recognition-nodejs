...
function drawFaces(canvas, data, fps, label) {
  console.log(label);
...
//     fullFaceDescription = await faceapi.detectSingleFace(img).withFaceLandmarks().withFaceDescriptor()
...
async function getLabelFaceDescriptions(labels) {
...
async function detectVideo(video, canvas) {
  ...
  .then(async result => {
    ...
    log(fullFaceDescriptions)
    try {
      // const fps = 1000 / (performance.now() - t0);
      // 0.6 is a good distance threshold value to judge
      // whether the descriptors match or not
      const maxDescriptorDistance = 0.6
      const faceMatcher = new faceapi.FaceMatcher(await getLabelFaceDescriptions(labels), maxDescriptorDistance);
      const recognitionResults = await fullFaceDescriptions.map(fd => faceMatcher.findBestMatch(fd.descriptor));
      ...
      recognitionResults.forEach((bestMatch, i) => {
        ...
        const fps = 1000 / (performance.now() - t0);
        drawFaces(canvas, result, fps.toLocaleString(), text);
        requestAnimationFrame(() => detectVideo(video, canvas));
        //   // const drawBox = new faceapi.draw.DrawBox(box, { label: text })
        //   // drawBox.draw(canvas)
        // drawFaces(canvas, bestMatch, fps.toLocaleString(), text);
        // requestAnimationFrame(() => detectVideo(video, canvas));
        // faceapi
        //   .detectAllFaces(video, optionsSSDMobileNet)
        //   .withFaceLandmarks()
